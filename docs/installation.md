...
`emsi-release` is distributed from GitHub Releases. Package managers should all
...
brew install emsi-release
```

The Homebrew formula is generated by GoReleaser from tagged releases. The tap
repository should be `MarlonJD/homebrew-tap`.

Direct install is also supported by downloading the matching
`emsi-release_<version>_darwin_<arch>.tar.gz` archive from GitHub Releases and
placing `emsi-release` on `PATH`.
...
scoop install emsi-release
```

```powershell
winget install MarlonJD.EmsiRelease
```
...
Direct install is also supported by downloading the matching
`emsi-release_<version>_windows_<arch>.zip` archive from GitHub Releases and
placing `emsi-release.exe` on `PATH`.
...
```bash
sudo dpkg -i emsi-release_<version>_linux_<arch>.deb
```

```bash
sudo rpm -i emsi-release_<version>_linux_<arch>.rpm
```
